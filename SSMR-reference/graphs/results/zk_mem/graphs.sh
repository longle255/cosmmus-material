...
tpvslatoutput=${dir}/plot_tpvslat.ps
tpdivlatoutput=${dir}/plot_tpdivlat.ps
...
set xlabel "Throughput (kcps)"
set xrange [0 : *]
set yrange [0 : 200]
set grid
set key bottom right

plot "$zkfile"     using (\$3 / 1e3):4 title "Zookeeper" with lines lc rgb "#000000" lw 2,\
     "$smrfile"    using (\$3 / 1e3):4 title "ZKsmr"     with lines lc rgb "#008B8B" lw 2,\
     "$ssmrfile1p" using (\$3 / 1e3):4 title "ZKssmr 1p" with lines lc rgb "#FF0000" lw 2,\
     "$ssmrfile2p" using (\$3 / 1e3):4 title "ZKssmr 2p" with lines lc rgb "#008000" lw 2,\
     "$ssmrfile4p" using (\$3 / 1e3):4 title "ZKssmr 4p" with lines lc rgb "#0000FF" lw 2,\
     "$ssmrfile8p" using (\$3 / 1e3):4 title "ZKssmr 8p" with lines lc rgb "#333333" lw 2

set output "$tpdivlatoutput"
set xlabel "Load (clients)"
set ylabel "Throughput/Latency (kcps/ms)"
set yrange [0 : *]

plot "$zkfile"     using 1:(\$3 / 1e3)/(\$4) title "Zookeeper" with lines lc rgb "#000000" lw 2,\
     "$smrfile"    using 1:(\$3 / 1e3)/(\$4) title "ZKsmr"     with lines lc rgb "#008B8B" lw 2,\
     "$ssmrfile1p" using 1:(\$3 / 1e3)/(\$4) title "ZKssmr 1p" with lines lc rgb "#FF0000" lw 2,\
     "$ssmrfile2p" using 1:(\$3 / 1e3)/(\$4) title "ZKssmr 2p" with lines lc rgb "#008000" lw 2,\
     "$ssmrfile4p" using 1:(\$3 / 1e3)/(\$4) title "ZKssmr 4p" with lines lc rgb "#0000FF" lw 2,\
     "$ssmrfile8p" using 1:(\$3 / 1e3)/(\$4) title "ZKssmr 8p" with lines lc rgb "#333333" lw 2
...
for graphfile in $tpoutput $tpvslatoutput $tpdivlatoutput $latoutput $cpuoutput $bwoutput $diskouttps $diskoutmbps
do
...
sizefile_max_tp=sizes_${rate}readrate_max_tp.txt
sizefile_tp75=sizes_${rate}readrate_tp75.txt
...
# set xtics ('0.00' 0.0, '0.25' 0.25, '0.50' 0.5, '0.75' 0.75, '1.00' 1.0)
set grid ytics
set key top center maxrows 2
set key samplen 2
#set key spacing 1.5
#set key width -3
plot "$sizefile_max_tp" using (\$3  / \$21):xtic(1)	title "Zookeeper"  fs pattern 1 lc rgb "black",\
     ''          using (\$12 / \$21):xtic(1)	title "ZKsmr"      fs solid lc rgb "#F5F5F5",\
...
set xtics auto
set ytics 20
plot "$sizefile_tp75" using (\$4 ):xtic(1)	title "Zookeeper"  fs pattern 1 lc rgb "black",\
     ''          using (\$13):xtic(1)	title "ZKsmr"      fs solid lc rgb "#F5F5F5",\
...
cdfoutput=plot_latency_cdfs_${size}bytes.ps
echo $cdfoutput
...
set terminal postscript eps enhanced color solid lw 2 "Helvetica" 27
...
set title "Latency CDF for $size bytes"
...
plot "$zkcdffile"     using (\$1 / 1e6):(\$2 / \$3)	title "Zookeeper" with lines lc rgb "#000000" lw 2.625,\
     "$smrcdffile"    using (\$1 / 1e6):(\$2 / \$3)	title "ZKsmr"     with lines lc rgb "#008B8B" lw 2.625,\
     "$ssmr1pcdffile" using (\$1 / 1e6):(\$2 / \$3)	title "ZKssmr 1p"  with lines lc rgb "#FF0000" lw 2.625,\
     "$ssmr2pcdffile" using (\$1 / 1e6):(\$2 / \$3)	title "ZKssmr 2p"  with lines lc rgb "#008000" lw 2.625,\
     "$ssmr4pcdffile" using (\$1 / 1e6):(\$2 / \$3)	title "ZKssmr 4p"  with lines lc rgb "#0000FF" lw 2.625,\
     "$ssmr8pcdffile" using (\$1 / 1e6):(\$2 / \$3)	title "ZKssmr 8p"  with lines lc rgb "#333333" lw 2.625
