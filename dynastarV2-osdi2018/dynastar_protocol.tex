...
In more detail, \dynastar\ distinguishes three types of commands:
...
the oracle forwards $C$ to that partition to execute. If the command 
accesses variables from multiple partitions, the oracle multicasts a 
$move(\omega,\ppm_d, C)$ command to the involved partitions to gather 
...
send reply to the client and returns the variables to their source. 
%Moving variables back to their source partitions help preserving the 
partitioning, thus make the cache on client efficient.
