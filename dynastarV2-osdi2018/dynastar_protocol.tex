...
%Moreover, partitions use a new partitioning computed by the oracle to relocate the state.
\end{itemize}
...
\dynastar defines a dynamic mapping of variables to partitions (i.e., each variable $v$ is mapped to a partition $\ppm$).
...
In the discussion in this section and in \S\ref{sec:detailed}, we assume that every command involves the oracle.
...
The oracle also collects hints from client commands and partitions to 
