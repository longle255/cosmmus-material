...
The code is publicly available with an open-source
...
It has 9 tables and five stored procedures that simulates a warehouse-centric 
...
all servers, considering that the ITEM table is not updated in this benchmark.
We implemented a Java version of TPC-C benchmark running on top of \dynastar. 
Each row in TPC-C tables is an object in \dynastar. The granularity is set to DISTRICT level,
thus each district or warehouse is a node in the graph. A transaction accesses a district 
belongs to a warehouse and creates a vertex between that district and the warehouse.
The objects (e.g., customers, orders, etc.) that belong to a district 
will be considered as a property of that district. However, if a transaction requires
