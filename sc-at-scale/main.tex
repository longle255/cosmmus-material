...
In this section, we present the results found for \appname{}\, a scalable social network application based on \ssmr{}, with different loads and partitionings and compare them to the performance when \ssmr{} signaling mechanism is turned off.
...
In Section~\ref{sec:evaluation:result}, we show the results.
...
All experiments are conducted on a cluster that had two types of nodes: (a) HP SE1102 nodes, equipped with two Intel Xeon L5420 processors running at 2.5 GHz and with 8 GB of main memory, and (b) Dell SC1435 nodes, equipped with two AMD Opteron 2212 processors running at 2.0 GHz and with 4 GB of main memory. The HP nodes were connected to an HP ProCurve 2920-48G gigabit network switch, and the Dell nodes were connected to another, identical switch. Those switches were interconnected by a 20 Gbps link.
...
Before each experiment, We kept the clocks synchronized using NTP in order to measure latency components involving events in different computers.
...
Thus enabling or disabling signal would not affect the performance of \appname{}.

In the Post workload, every command accesses up to all partitions in the system, which requires partitions to exchange both data and signal. 
On the test with only one partition, obviously no signal or data is being exchanged, the command behaves as a single partition command.
From two partitions, signal and data are being exchanged among partitions. However, we can see that the signaling doesn't affect the performance of \appname{} significantly. This happens because the Post command changes the content of the User object (changes in timeline), so on both tests, partitions have to exchange the data. Then even when signaling is turned off, the partitions still have to wait for the data. As a result, the throughput and delay of signaling turned on or off are not much different.
