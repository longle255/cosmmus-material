...
%% partitioning and replication (e.g., \cite{x,y,z,w}).  By partitioning the service state
...
%% to be revisited as the system evolves. 


%% Although state partitioning and replication are commonly used used to
%% achieve scalability and high availability (e.g., \cite{x,y,z,w}),
%% there is relatively little prior work on state partitioning in the
%% context of state machine replication.
...
%% To submit a
%% command, clients consult the location oracle to learn the partitions
%% it must contact.  If the command accesses objects in multiple
%% partitions, the oracle updates the workload graph with the command and
%% moves objects to a single destination partition, after which the
%% client can multicast the command.  In order to choose the ``right"
%% destination partition for the objects of a command, the oracle
%% periodically computes a partitioning of the workload graph with METIS
%% and uses the resulting partitioning as a reference to decide where to
%% move the objects to.  The oracle uses two heuristics to decide the
%% destination partition of the objects of a command.  First, it tries to
%% move the minimum number of objects.  Second, it attempts to respect as
%% much as possible the partitioning of the workload graph.



%% The client queries the oracle with command.
%% Partitions are replicated, so all commands are multicast.
%% It the oracle sees that the command involves multiple partitions.
%% It does the following:
%% It contacts the partitions and have them move data. Then responds
%% to the client with the single partition.
%% The partition may fail because something may fail. It may consult the
%% cache.

%% Executing as a multi-partition command is more expensive than
%% moving the data.
...
%% starts and re-partitioning the state requires system shutdowns.
%% Second,  \dynastar does not require any a priori knowledge about the workload.

%% \dynastar is inspired by the optimized static
