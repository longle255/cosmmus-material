...
%Borrow from DSSMR paper
%iState machine replication (SMR) is a well-known technique that guarantees strong consistency (i.e., linearizability) to online services.
%In SMR, client commands are executed in the same order on all server replicas, and after executing each command, every replica reaches the same state.
...
%%\fxnote[draft]{Change ssmr $\rightarrow$ S-SMR}
