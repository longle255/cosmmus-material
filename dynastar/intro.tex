...
in advance.  A prominent example of a static scheme is
S-SMR~\cite{bezerra2014ssmr}. The major problem with static schemes,
though, is that they do not adapt as workloads change. Therefore, over
time, performance is likely to degrade (e.g., in social networks users
join and leave the system, connections are created and removed).

In contrast, dynamic schemes assume no prior knowledge of the
workload, and adapt the partitioning scheme over time.  A prominent
example of a dynamic scheme is DS-SMR~\cite{hoang2016}. DS-SMR moves
data to partitions on-demand in order to avoid multi-partition
commands. While the overhead due to moving data is comparable to the
overhead of executing multi-partition commands, the strategy of moving
data is beneficial if future commands access the same partitions.

The problem for dynamic schemes is how to select the partition to
which to move data. Figure~\ref{fig:motivation} shows the results of
an experiment evaluating two different strategies. The first, which we
refer to as \emph{decentralized dynamic}, is the strategy implemented
by DS-SMR.  DS-SMR selects partitions randomly. This allows for a
completely decentralized implementation, in which partitions make only
local choices about data movement. As Figure~\ref{fig:motivation-a}
shows, this approach works well for data with strong locality, but
performs poorly for data with weak locality.

In contrast, an ''optimal'' dynamic scheme would re-compute a
partitioning on every change in workload using a centralized, global
view of the network.  Figure~\ref{fig:motivation-b} shows the
performance for such a scheme.  In practice, such a scheme would be
impossible to implement, since it would require perfectly predicting
the workload, and constantly moving the state.
...
perfect dynamic scheme. The key idea behind \dynastar is that it
