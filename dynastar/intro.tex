...
workload graph using the METIS partitioner. \eb{I still don't get this... Does this mean that an object is relocated potentially everytime a command is executed? If so, how is this different from DS-SMR? Also, do you need to rellocate all objects accessed by each command to the same partition? An optimal partitioning for a certain workload doesn't imply that every command is executed against a single parition...}
