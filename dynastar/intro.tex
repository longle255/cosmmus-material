...
partitioning: \emph{static} and \emph{dynamic}. Static schemes (e.g.,
S-SMR~\cite{bezerra2014ssmr}) choose an immutable assignment of
objects to partitions in advance. In contrast, dynamic schemes assume
no prior knowledge of the workload, and adapt the partitioning scheme
as workloads change over time (e.g., in social networks users join and
leave the system, connections are created and removed). Typically,
this is implemented by moving data ``on demand'' to a single partition
which executes a user command.

The major challenge, therefore, in designing a dynamic scheme is
determining how the system selects the partition to which to move
data.  Figure~\ref{fig:motivation} shows the result of a motivating
experiment that highlights two approaches. In the experiment, we show
the throughput and number of state moves over time with two different
workloads; one with strong locality and one with weak locality. For
brevity, we postpone the details of the experimental setup until
Section~\ref{sec:experiments}.

The first approach, which we refer to as \emph{decentralized dynamic},
is the strategy implemented by DS-SMR.  DS-SMR selects partitions
randomly. This allows for a completely a decentralized implementation,
in which partitions make only local choices about data movement. As
Figure~\ref{fig:motivation-a} shows, this approach works well for data
with strong locality, but is unstable for workloads with weak
locality, since data is constantly moved from one partition to another.


The second approach, which we refer to as \emph{optimal dynamic},
re-computes a partitioning on every change in workload using a
centralized, global view of the network. In practice, such a scheme
would be impossible to implement, since it would require perfectly
predicting the workload. Nevertheless, the results in
Figure~\ref{fig:motivation-a} are useful, as they show an upper-bound
for the best throughput one could hope to achieve.
...
optimal dynamic scheme. The key idea behind \dynastar is that it
