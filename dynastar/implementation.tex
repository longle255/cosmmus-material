...
\subsection{Social network service}
\label{sec:imp:\appname}

For the purposes of our evaluation, we have developed a Twitter-like
social network service using the \dynastar{} library.  In our social network, 
users can follow, unfollow, post, or read other users'
...
our service so that getTimeline are always a single-partition command.
...
repositories.\footnote{https://bitbucket.org/usi-dslab/ds-smr} Since all three systems (S-SMR, DS-SMR, and \dynastar) provide
a common application-level interface, there was no additional work required
to port the social network application. 
%\rjs{We should add links to the repositories.}
