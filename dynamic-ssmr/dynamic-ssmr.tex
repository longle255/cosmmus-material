...
For instance, in Figure~\ref{fig:cache_retry} the client has an out-of-date cache, incurring in a new consultation to the oracle when executing $C_1$.
...
Besides, if most commands are executed without consulting the oracle, as in the case of $C_2$ in Figure~\ref{fig:cache_retry}, we avoid turning the oracle into a bottleneck.
