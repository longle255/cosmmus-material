...
If the reply received to the command is $retry$, the procedure restarts: the proxy consults the oracle, possibly moves variables across partitions, and multicasts $C$ to the appropriate partitions again.
...
One of the purposes of the oracle proxy is to tell client proxies where each variable is located, so that commands can be multicast to the right partitions.
...
The prophecy regarding a $delete(v)$ contains the partition where $v$ is located, or $ok$, in case $v$ already does not exist.
...
If $v \in \ppm$ or $v$ does not exist, the oracle tells the client that the delete command suceeded.
