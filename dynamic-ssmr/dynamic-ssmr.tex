...
Dynamic S-SMR improves on S-SMR implementation by adding a central oracle that has the information of all state variables in $\vvm$ and provides a mechanism of controlling the access to those variable that ensure linearizability. We distinguish between three operation types: $update(v)$, an operation that updates the value of a state variable, $v$, $create(v, P)$, an operation that create a state variable at a specific partition P, and $move(v, \ppm_n)$, an operation that move $v$ to partition $\ppm_n$.
