...
Application designers who use \dynastar
 ...
 \item[--] \emph{ProxyClient}: \lle{provides the communication between application client and server, while encapsulating all 
...
\lle{Application designer can configure the oracle to trigger repartitioning in different ways: based on the number of changes to the graph, or based on some interval of time.}
...
\lle{
The TPC-C workload depicts the activity of a wholesale supplier company.
...
\lle{The ITEM table is replicated to all servers, considering that the ITEM table is not updated in this benchmark.}
