...
\lle{Therefore it is aimed be the catalyst that supports the widespread adoption
of blockchain technologies.}

\lle{Blockchain analysis is becoming increasingly important in the
...
blockchain \cite{chainalysis2020kyt, bloxy, blocmonitor}. Private company use
blockchain analysis solutions to track its customers’ transactions to learn more
...
off-load expensive computation to a side-chain to optimize the cost. \lle {Our
...
blockchains or the applications.}
...
\lle{Our proposed service could be quickly and easily integrated with existing
blockchains, DApps, or non-blockchain based applications. Our solution doesn't
require any changes in the blockchain code, makes the integration simple without
hassle. The system is accessible in different ways. On-chain smart contract or
DApps can request necessary information by issuing a call to our on-chain
components. Non-blockchain based application can also access to our data with
REST API. In addition, the design of our system is yet flexible enough to add
support for new blockchain.}

\lle{Our main source of revenue is from blockchain's users. Users will be
charged based on the time it takes to execute their queries (for instance, a query
for the location of objects results in a lower cost than a query for calculating
an optimized location). With our pay-per-call pricing model, the users only pay
for the queries that they run. This payment model has several benefits. There is
no barrier to entry for users, no commitment required, and it offers the users more
flexibility around the services they want, and when they want them. For instance,
the application could decide to cache some queries results once in a while for
further usages to optimize costs.}
...
\lle{To fulfill the goals of the project, the proposed system needs to address
...
\begin{itemize}
      ...
      principals, their implementations and data structure are different. The
      ...
      should be "plug-and-play", without introducing any new change in the
...
OSC is also responsible for computing the fee of each request. \lle{Figure
...
\lle{The project plan will divide the research activities described in Section
...
\lle{\textbf{Phase 1: Developing off-chain component.} In this phase, we aim to develop
...
\lle{\textbf{Phase 2: Developing on-chain smart contracts.} Similar to Phase 1, we
...
\lle{\textbf{Phase 3: Deploying and evaluating.} We will deploy the system on the supported
...
\lle{\textbf{Risks.} The proposed project is adventurous, as with any research
