...
object. \lle{The advantage of linearizability is its ``local'' and ``non-blocking''
properties. Linearizability is local meaning that it is sufficient for a system
to linearize operations for each individual object to achieve global
linearizability. Linearizability is non-blocking meaning that pending
...
number of crossing edges those subgraphs. \lle{Intuitively, a good partitioning is
...
the locality of a workload is characterized by the data accessing patterns
produced by the application. The workloads have \emph{strong
