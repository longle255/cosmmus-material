...
\ENDINDENT
\vspace{1.0mm}
\STATE{Each client has structure $next[p]$ with the next entry in $p$'s buffer}
\vspace{1.0mm}
...
\INDENT{Each server $p$ at group $g$ also has:}
	...
	\STATE{$Leader[\ ]$ the identifier of the leader at $g$ (as seen by $p$)}
...
\vspace{1.0mm}
	\STATE for each $h$ in $dst$, for each $q$ in $h$: increment $ptr[q]$
