...
We coordinate the execution within each group using the leader-follower model (i.e., also known as primary-backup model).
Each server process of \libname organizes its memory in memory regions: a shared
memory region and a protected memory spaces. All processes have remote access
(read/write) to the shared memory space. 
Only one process (leader) of each group
