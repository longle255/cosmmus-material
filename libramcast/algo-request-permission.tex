...
\WHEN[\textbf{Task 6}]{suspect $Leader[g]$ \band\ $p$ is $g$'s next leader}	
...
\vspace{2.0mm}		
\WHEN[\textbf{Task 7}]{receive responses $(\textsf{granted}, pend)$ from \\
...
\WHEN[\textbf{Task 8}]{receive $(\textsf{req\_perm}, round)$ from $q$ in $h$}	
	\IF{$round > Round[h]$}
...
\WHEN[\textbf{Task 9}]{suspect client $c$}	
